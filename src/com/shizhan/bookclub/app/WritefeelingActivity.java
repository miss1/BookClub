...
public class WritefeelingActivity extends Activity implements OnClickListener{
			...
			if(!content.equals("")){                //发表的内容不为空，则接下来获取服务器时间
				Bmob.getServerTime(this, new GetServerTimeListener() {
					...
					public void onSuccess(long arg0) {
						...
						MyUsers user = BmobUser.getCurrentUser(WritefeelingActivity.this, MyUsers.class);
						//创建帖子信息
						Post post = new Post();
						if(!title.equals("")){
							post.setTitle(title);
						...
						post.save(WritefeelingActivity.this, new SaveListener() {
							...
							public void onSuccess() {
								Toast.makeText(WritefeelingActivity.this, "帖子发表成功", Toast.LENGTH_SHORT).show();
								finish();
							}
							
							@Override
							public void onFailure(int arg0, String arg1) {
								Toast.makeText(WritefeelingActivity.this, arg1, Toast.LENGTH_SHORT).show();								
							}
						});
					}
				...
				Toast.makeText(WritefeelingActivity.this, "帖子内容不能为空", Toast.LENGTH_SHORT).show();
			}
			break;
		default:
			break;
